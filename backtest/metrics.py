...
def calculate_cagr(df):
    ...
    """
    beginning_value = df['cumulative_returns'].iloc[0]
    ending_value = df['cumulative_returns'].iloc[-1]
...
def win_rate(df):
    ...
    Calculate win_rate
    """
    df['win'] = np.where(df['returns'] > 1, 1, 0)
    return df['win'].mean()
...
def max_drawdown(df):
    ...
    df['Drawdown'] = (df['cumulative_returns'] - df['Running_Max'])/df['cumulative_returns']

    # Calculate the maximum drawdown
    max_drawdown = df['Drawdown'].min()
    return max_drawdown

def max_consecutives_losses(df):
    """
    Calculate max number of losing trades in a row
    """
    # Create a condition for values above 1
    condition = df['returns'] < 1

    # Calculate the consecutive groups
    consecutive_groups = (condition != condition.shift()).cumsum()

    # Filter the groups where values are above 1
    above_1_groups = consecutive_groups[condition]

    # Count the maximum number of consecutive occurrences
    max_consecutives_losses = above_1_groups.value_counts().max()

    return max_consecutives_losses

def sharpe(df, cagr):
    df.set_index('date', inplace=True)

    # Calculate the standard deviation of returns
    std_dev = calculate_annualized_volatility(df['returns'])

    # Calculate the Sharpe Ratio
    sharpe_ratio = (cagr - benchmark_rate) / std_dev
...
def calculate_annualized_downside_deviation(return_series: pd.Series) -> float:
    """
...
def calculate_sortino_ratio(return_series, cagr):
    """
...
def calculate_jensens_alpha_v2(return_series: pd.Series) -> float:
    ...
    """
    #spy_data = load_spy_data() IL FAUT RECUPERER LES DATA AUTRMEENTS CAR LEUR LIB MARCHE PAS
